...
* Your task is to create a Python script that analyzes the records to calculate each of the following:
  ...
  * The total number of months included in the dataset
  loop through the months and count how many months there are 

  * The net total amount of "Profit/Losses" over the entire period
  loop though Profit/losses and keep adding 
  we open the file with open csv with csv reader object 
  then we need to do next so we skip over the header
  then create a for loop or do a list comprehension(do this) that adds all the stuff up togeher 
  basically creating a counter

  * The average of the changes in "Profit/Losses" over the entire period

  * The greatest increase in profits (date and amount) over the entire period
they mean month over month, 
needs to have a print statement
need to create a with open output.txt in "w" mode then write the answers and save it into a text file 
...
when we iterate through Voter ID, since voter ID accounts for 1 vote; instead of saying total_votes= previous total votes + row, we can just add 1 instead of row. make sure our counter is only incrementing by one and not total Voter ID count

  * A complete list of candidates who received votes
 loop though row[0] and for every single row count 1
  if row 2 is "Khan" then we add 1 to the Khan counter
  create bucket variables for the candidates we can do all this in one loop
